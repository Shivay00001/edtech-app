...
Write-Host "║   EdTech App - Mobile Learning         ║" -ForegroundColor Cyan
...
if ($Install -or -not (Test-Path "$EdTechDir\.dart_tool")) {
...
if (-not (Test-Path "$EdTechDir\.env")) {
    if (Test-Path "$EdTechDir\.env.example") {
        Copy-Item "$EdTechDir\.env.example" "$EdTechDir\.env"
        Write-Success "Created .env file (please configure Firebase)"
    }
}
...
switch ($Platform) {
    ...
    "web" {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # Serve web build
            Write-Host ""
            Write-Info "Starting web server on port 3004..."
            cd build\web
...
Write-Success "EdTech App Deployment Complete"
Write-Host "═══════════════════════════════════════" -ForegroundColor Green
